package org.firstinspires.ftc.teamcode;

public class Subsystem_Constants {
    //arms
    public final static double highBasket1 = 102.3167; //97.854286777;
    public final static double highRung1 = 3.3954; //2.5585;
    public final static double highRung1_2 = 42.4905; //37.6126; //49.1587;

    public final static double wall1 = 15.0642;
    public final static double wall1_2 = 32.0412; //33.3803; //not tested
    public final static double lowBasket1 = 50; //not tested
    public final static double floor1 = 0.6217;
    public final static double down1 = 4.48338159887;
    public final static double sub1 = 8.1777; //6.4322;
    public final static double highBasket2 = 141.1539; //131.0525;
    public final static double highRung2 = 91-0.722; //91; //90.7355; //90.2924; // 88.72; //87.1025; //90.381; //91.7102;//94.457; //90.381; //92.6;
    public final static double highRung2_2 = 33.76; //38.3676; //37.1271; //31.1093;
    public final static double wall2 = 156.4382; //157.9896;// 154.8883; //157.0149; //154.8883; //156.749
    public final static double wall2_2 = 168.9771;
    public final static double lowBasket2 = 50; //not tested
    public final static double floor2 = 161.3567; //159.8503;
    public final static double down2 = 10.0199819357;
    public final static double sub2 = 154.721207477;

    //servos
    public final static double clawScale0 = 0.45;
    public final static double clawScale1 = 1;
    public final static double closeClaw = 1;
    public final static double openClaw = 0;

    public final static double intake_AngleScale0 = 0;
    public final static double intake_AngleScale1 = 1;
    public final static double intake_AngleFloor = 0.26;
    public final static double intake_AngleBasket = 0.36;
    public final static double intake_AngleRung = 0.2;
    public final static double intake_AngleStart = 0.8;
    public final static double intake_AngleWall = 0;
    public final static double intake_AngleVertical = 0.65;

    public final static double claw_AngleScale0 = 0;
    public final static double claw_AngleScale1 = 0.7;
    public final static double claw_AngleForward = 0.0571;
    public final static double claw_AngleBackward = 1;
    public final static double claw_AngleLeft = 0;

    public final static double sweeperScale0 = 0.085;
    public final static double sweeperScale1 = 0.482;
    public final static double closeSweeper = 0;
    public final static double openSweeper = 0.8;
}
